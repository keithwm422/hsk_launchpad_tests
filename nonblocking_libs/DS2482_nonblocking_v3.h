...
private:
	...
	uint8_t mTimeout;
	uint8_t readByte();
	bool setReadPtr(uint8_t readPtr);
        bool writeByte(uint8_t byte_to_write);
        bool writeTwoBytes(uint8_t byte_1, uint8_t byte_2);
	//uint8_t OneWB(bool setReadPtr=false); //blocks until
  //bool busyWait(bool setReadPtr=false); //blocks until
