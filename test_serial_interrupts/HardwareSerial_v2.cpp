...
// Constructors ////////////////////////////////////////////////////////////////
HardwareSerial_v2::HardwareSerial_v2(void)
{
    txWriteIndex = 0;
    txReadIndex = 0;
    rxWriteIndex = 0;
...
HardwareSerial_v2::HardwareSerial_v2(unsigned long module) 
{
    txWriteIndex = 0;
    txReadIndex = 0;
    rxWriteIndex = 0;
...
// Private Methods //////////////////////////////////////////////////////////////
void
HardwareSerial_v2::flushAll(void)
{
...
void
HardwareSerial_v2::primeTransmit(unsigned long ulBase)
{
    //
...
void
HardwareSerial_v2::begin(unsigned long baud)
{
	baudRate = baud;
    //
    // Initialize the UART.
    ...
    // Enable interrupts
    //
    //ROM_UARTIntEnable(UART_BASE, UART_INT_RX | UART_INT_RT | UART_INT_OE | UART_INT_BE | UART_INT_FE);
...
void
HardwareSerial_v2::setBufferSize(unsigned long txsize, unsigned long rxsize)
{
    if (txsize > 0)
        txBufferSize = txsize;
    if (rxsize > 0)
...
void
HardwareSerial_v2::setModule(unsigned long module)
{
...
void 
HardwareSerial_v2::setPins(unsigned long pins)
{
	if(pins == UART1_PORTB)
	{
...
HardwareSerial_v2::operator bool()
{
	return true;
}

void HardwareSerial_v2::end()
{
...
int HardwareSerial_v2::read(void)
{
    if(RX_BUFFER_EMPTY) {
    	return -1;
    }
...
void HardwareSerial_v2::flush()
{
    while(!TX_BUFFER_EMPTY);
    while (ROM_UARTBusy(UART_BASE)) ;
}

size_t HardwareSerial_v2::write(uint8_t c)
{
...
void HardwareSerial_v2::UARTIntHandler(void){
...
void HardwareSerial_v2::UARTIntHandler_0(void){
    unsigned long ulInts;
    long lChar;
    // Get and clear the current interrupt source(s)
    //
    ...
    if(ulInts &(UART_INT_OE | UART_INT_BE | UART_INT_FE)){
           //ROM_UARTDisable(UART_BASE);
           turn_off_TX=true; 
    }
}
void HardwareSerial_v2::turn_back_on_UART(void){
  //ROM_UARTEnable(UART_BASE);
  turn_off_TX=false;
}

void
vUARTIntHandler(void)
{
    vSerial.UARTIntHandler();
}

void
vUARTIntHandler1(void)
{
    vSerial1.UARTIntHandler();
}

void
vUARTIntHandler2(void)
{
    vSerial2.UARTIntHandler();
}

void
vUARTIntHandler3(void)
{
    vSerial3.UARTIntHandler();
}

void
vUARTIntHandler4(void)
{
    vSerial4.UARTIntHandler();
}

void
vUARTIntHandler5(void)
{
    vSerial5.UARTIntHandler();
}

void
vUARTIntHandler6(void)
{
    vSerial6.UARTIntHandler();
}

void
vUARTIntHandler7(void)
{
    vSerial7.UARTIntHandler();
}

void vserialEvent() __attribute__((weak));
void vserialEvent() {}
void vserialEvent1() __attribute__((weak));
void vserialEvent1() {}
void vserialEvent2() __attribute__((weak));
void vserialEvent2() {}
void vserialEvent3() __attribute__((weak));
void vserialEvent3() {}
void vserialEvent4() __attribute__((weak));
void vserialEvent4() {}
void vserialEvent5() __attribute__((weak));
void vserialEvent5() {}
void vserialEvent6() __attribute__((weak));
void vserialEvent6() {}
void vserialEvent7() __attribute__((weak));
void vserialEvent7() {}

void serialEventRun(void)
{
    if (vSerial.available()) vserialEvent();
    if (vSerial1.available()) vserialEvent1();
    if (vSerial2.available()) vserialEvent2();
    if (vSerial3.available()) vserialEvent3();
    if (vSerial4.available()) vserialEvent4();
    if (vSerial5.available()) vserialEvent5();
    if (vSerial6.available()) vserialEvent6();
    if (vSerial7.available()) vserialEvent7();
}

HardwareSerial_v2 vSerial;
HardwareSerial_v2 vSerial1(1);
HardwareSerial_v2 vSerial2(2);
HardwareSerial_v2 vSerial3(3);
HardwareSerial_v2 vSerial4(4);
HardwareSerial_v2 vSerial5(5);
HardwareSerial_v2 vSerial6(6);
HardwareSerial_v2 vSerial7(7);
