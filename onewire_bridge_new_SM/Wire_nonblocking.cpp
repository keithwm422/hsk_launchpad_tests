...
// for nonblocking
/*uint8_t TwoWire::txProcessState = 0;
uint8_t TwoWire::rxProcessState = 0;
uint8_t TwoWire::rxBuffer[BUFFER_LENGTH];
uint8_t TwoWire::rxReadIndex = 0;
uint8_t TwoWire::rxWriteIndex = 0;
uint8_t TwoWire::txAddress = 0;
//initialize rx nonblocking vars used
uint8_t TwoWire::rxBytesRemaining = 0;
uint8_t TwoWire::rxBytesReceived = 0;
...
// use only even! sendStop is embedded in the bottom
enum rxProcessState_enum {
  RX_IDLE = 0,
  RX_WAIT_BUS_BUSY = 2,
  RX_WAIT_AFTER_DATA = 4,
  RX_WAIT_AFTER_STOP = 6
};
enum txProcessState_enum {
  TX_IDLE = 0,
  TX_WAIT_FOR_BUS_NBUSY = 1,
  TX_WAIT_FOR_NBUSY = 2,
  TX_WAIT_AFTER_ADDRESS = 3,
  TX_WAIT_AFTER_DATA = 4,
  TX_WAIT_BEFORE_STOP = 5,
  TX_WAIT_AFTER_STOP = 6
};
// Constructors ////////////////////////////////////////////////////////////////
...
uint8_t TwoWire::getRxStatus(uint8_t quantity){
  unsigned long cmd = 0;
  uint8_t sendStop = rxProcessState & 0x01;
  switch(rxProcessState &0xFE){
    case RX_IDLE: return 0;
    case RX_WAIT_BUS_BUSY: 
       if (currentState == IDLE) if (ROM_I2CMasterBusBusy(MASTER_BASE)) return I2C_BUSY;
      if((quantity > 1) || !sendStop) cmd = RUN_BIT | START_BIT | ACK_BIT;
      else cmd = RUN_BIT | START_BIT | (sendStop << 2);
      HWREG(MASTER_BASE + I2C_O_MCS) = cmd;
      rxProcessState = RX_WAIT_AFTER_DATA | sendStop;
    case RX_WAIT_AFTER_DATA:
      while(1) {
        if (ROM_I2CMasterBusy(MASTER_BASE)) return I2C_BUSY;
        uint8_t error = ROM_I2CMasterErr(MASTER_BASE);
        if (error != I2C_MASTER_ERR_NONE) {
          ROM_I2CMasterControl(MASTER_BASE, I2C_MASTER_CMD_BURST_RECEIVE_ERROR_STOP);
          rxProcessState = RX_IDLE;
          return 0;
        }
        if(rxBytesRemaining){
          rxBuffer[rxWriteIndex] = ROM_I2CMasterDataGet(MASTER_BASE);
        ...
        if (!rxBytesRemaining) {
          if (sendStop) {
            ...
            if (ROM_I2CMasterBusy(MASTER_BASE)) {
              rxProcessState = RX_WAIT_AFTER_STOP;
              return I2C_BUSY;
            }
            currentState = IDLE;
            rxProcessState = RX_IDLE;
        ...
        rxProcessState = RX_WAIT_AFTER_DATA | sendStop;
        if (rxBytesRemaining == 1) cmd = RUN_BIT;
        else cmd = RUN_BIT | ACK_BIT;
        HWREG(MASTER_BASE + I2C_O_MCS) = cmd;     
      }
    case RX_WAIT_AFTER_STOP:
      if (ROM_I2CMasterBusy(MASTER_BASE)) return I2C_BUSY;
      currentState = IDLE;
      rxProcessState = RX_IDLE;
      return rxBytesReceived;
...
//new requestFrom_nonblocking
uint8_t TwoWire::requestFrom_nonblocking(uint8_t address, uint8_t quantity, uint8_t sendStop){
  uint8_t error = 0;
  ...
  rxBytesRemaining = quantity;
  rxBytesReceived = 0;
  rxProcessState = RX_WAIT_BUS_BUSY | sendStop;
  //
...
// up to this is same as patrick's
// here patrick does return getRxStatus(quantity); and everything below is gone.
  return getRxStatus(quantity);
}

uint8_t TwoWire::requestFrom(uint8_t address, uint8_t quantity, uint8_t sendStop) {
  if (!quantity) return 0;
  uint8_t error = requestFrom_nonblocking(address,quantity,sendStop);
  if (error) return error;
  while ((error = getRxStatus(quantity)) == I2C_BUSY);
  return error;
}
  /*
uint8_t TwoWire::getRxData(unsigned long cmd) {
  ...
  HWREG(MASTER_BASE + I2C_O_MCS) = cmd;
  // this is the end of RX_WAIT_BUS_BUSY case
   // Work-around of I2C MasterBUSY Status bit does not get set Immediately
   // See ERRATA I2C#08 in http://www.ti.com/lit/er/spmz850g/spmz850g.pdf
   //
  // should be start of next case here
    while(!(HWREG(MASTER_BASE + I2C_O_MRIS) & I2C_MRIS_RIS));
    HWREG(MASTER_BASE + I2C_O_MICR) |= I2C_MICR_IC;
  uint8_t error = ROM_I2CMasterErr(MASTER_BASE);
  if (error != I2C_MASTER_ERR_NONE) {
...
// here patrick does return getRxStatus(quantity); and everything below is gone.
  ...
  currentState = MASTER_RX;
 // still in but totally matching rx_wait_after_data
...
uint8_t TwoWire::endTransmission_nonblocking(uint8_t sendStop) {
  if (TX_BUFFER_EMPTY) return 0;
  if (txProcessState != TX_IDLE) return 5;
  txProcessState = TX_WAIT_FOR_BUS_NBUSY;
  // embed sendStop in the top bit
  if (sendStop) txProcessState |= 0x80;
  return getTxStatus();
}

uint8_t TwoWire::getTxStatus() {
  uint8_t sendStop;
  sendStop = txProcessState & 0x80;
  // Note that there are no break statements here - we'll fall through
  // the state machine if no waits ever occur (which is impossible... hence the nonblocking function!)
  switch(txProcessState & 0x7F) {
    case TX_IDLE: return 0;
    case TX_WAIT_FOR_BUS_NBUSY: if (ROM_I2CMasterBusBusy(MASTER_BASE)) return I2C_BUSY; txProcessState = TX_WAIT_FOR_NBUSY | sendStop;
    case TX_WAIT_FOR_NBUSY: if (ROM_I2CMasterBusy(MASTER_BASE)) return I2C_BUSY;
      ROM_I2CMasterSlaveAddrSet(MASTER_BASE, txAddress, false);
      txProcessState = TX_WAIT_AFTER_ADDRESS | sendStop;
    case TX_WAIT_AFTER_ADDRESS: if (ROM_I2CMasterBusy(MASTER_BASE)) return I2C_BUSY;
      ROM_I2CMasterDataPut(MASTER_BASE, txBuffer[txReadIndex]);
      HWREG(MASTER_BASE + I2C_O_MCS) = RUN_BIT | START_BIT;
      txProcessState = TX_WAIT_AFTER_DATA | sendStop;
    case TX_WAIT_AFTER_DATA:
      // this is a nonblocking loop, note the returns
      do {
        if (ROM_I2CMasterBusy(MASTER_BASE)) return I2C_BUSY;
        // completed, so increment buffer pointer
        txReadIndex = (txReadIndex + 1) % BUFFER_LENGTH;      
        uint8_t error = ROM_I2CMasterErr(MASTER_BASE);
        // bail on error.
        if (error != I2C_MASTER_ERR_NONE) {
          ROM_I2CMasterControl(MASTER_BASE, I2C_MASTER_CMD_BURST_SEND_ERROR_STOP);
          txProcessState = TX_IDLE;
          return error;
        }
        // start sending the next one
        if (!TX_BUFFER_EMPTY) {
          ROM_I2CMasterDataPut(MASTER_BASE, txBuffer[txReadIndex]);
          HWREG(MASTER_BASE + I2C_O_MCS) = RUN_BIT;
        }
      } while (!TX_BUFFER_EMPTY); // jump back to the beginning of TX_WAIT_AFTER_DATA, bailing if master busy
      // all bytes transmitted, bus not busy any more. do we need to sendStop?
      if (!sendStop) {
        // no, we're done
      ...
      // don't care about sendStop anymore
      txProcessState = TX_WAIT_BEFORE_STOP;
    case TX_WAIT_BEFORE_STOP:
      if (ROM_I2CMasterBusy(MASTER_BASE)) return I2C_BUSY;
      HWREG(MASTER_BASE + I2C_O_MCS) = STOP_BIT;
      txProcessState = TX_WAIT_AFTER_STOP;
    case TX_WAIT_AFTER_STOP:
      if (ROM_I2CMasterBusy(MASTER_BASE)) return I2C_BUSY;
      currentState = IDLE;
      transmitting = 0;
      txProcessState = TX_IDLE;
